...
impl StringOps {
    ...
    pub fn str_upper_impl(frame: &TinyFrame, column: &str) -> PyResult<TinyFrame> {
        ...
        let result_values: Vec<Option<String>> = values
            .iter()
        ...
        new_columns.insert(
            format!("{}_upper", column),
            TinyColumn::OptStr(result_values),
        );
    ...
    pub fn str_split_impl(frame: &TinyFrame, column: &str, delimiter: &str) -> PyResult<TinyFrame> {
        let values = Self::extract_string_values(frame, column)?;
        let result_values: Vec<Option<Vec<String>>> = values
            .iter()
            .map(|opt_str| {
                opt_str
                    .as_ref()
                    .map(|s| s.split(delimiter).map(|part| part.to_string()).collect())
            })
            .collect();
    ...
    pub fn str_contains_impl(
        frame: &TinyFrame,
        column: &str,
        substring: &str,
    ) -> PyResult<TinyFrame> {
        let values = Self::extract_string_values(frame, column)?;
        let result_values: Vec<Option<bool>> = values
            .iter()
        ...
        let mut new_columns = frame.columns.clone();
        new_columns.insert(
            format!("{}_contains", column),
            TinyColumn::OptBool(result_values),
        );
    ...
    pub fn str_len_impl(frame: &TinyFrame, column: &str) -> PyResult<TinyFrame> {
        let values = Self::extract_string_values(frame, column)?;
        let result_values: Vec<Option<i64>> = values
            .iter()
    ...
    pub fn str_cat_impl(frame: &TinyFrame, column: &str, separator: &str) -> PyResult<TinyFrame> {
        ...
        new_columns.insert(
            format!("{}_cat", column),
            TinyColumn::Str(vec![concatenated; frame.length]),
        );
