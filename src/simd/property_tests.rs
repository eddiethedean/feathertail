#[cfg(test)]
mod property_tests {
    use crate::simd::{SimdOps, SimdStringOps};
    ...
    fn test_sum_commutativity() {
        ...
        for (a, b) in test_cases {
            ...
            assert!(
                (sum_combined - (sum_a + sum_b)).abs() < 1e-10,
                "Sum commutativity failed for a: {:?}, b: {:?}",
                a,
                b
            );
        }
    ...
    fn test_sum_associativity() {
        ...
        assert!(
            (sum_combined - (sum_a + sum_b + sum_c)).abs() < 1e-10,
        ...
        for data in test_cases {
            if !data.is_empty() {
                ...
                for &value in &data {
                    assert!(
                        value >= min,
                        "All values should be >= min for data: {:?}",
                        data
                    );
                    assert!(
                        value <= max,
                        "All values should be <= max for data: {:?}",
                        data
                    );
                }
            }
    ...
    fn test_std_dev_properties() {
        ...
        for data in test_cases {
            if data.len() >= 2 {
                ...
                assert!(
                    std_dev >= 0.0,
                    "Standard deviation should be >= 0 for data: {:?}",
                    data
                );
                assert!(
                    (std_dev * std_dev - variance).abs() < 1e-10,
                    "std_dev^2 should equal variance for data: {:?}",
                    data
                );
            }
        }
    ...
    fn test_dot_product_properties() {
        ...
        for (a, b) in test_cases {
            ...
            assert!(
                (dot_ab - dot_ba).abs() < 1e-10,
                "Dot product should be commutative for a: {:?}, b: {:?}",
                a,
                b
            );
        }
    ...
    fn test_dot_product_distributivity() {
        ...
        assert!(
            (dot_a_bc - dot_ab_plus_ac).abs() < 1e-10,
    ...
    fn test_add_commutativity() {
        ...
        for (a, b) in test_cases {
            ...
            for (i, (x, y)) in add_ab.iter().zip(add_ba.iter()).enumerate() {
                assert!(
                    (x - y).abs() < 1e-10,
                    "Addition should be commutative at index {}: a: {:?}, b: {:?}",
                    i,
                    a,
                    b
                );
    ...
    fn test_add_associativity() {
        ...
        for (i, (x, y)) in abc.iter().zip(abc2.iter()).enumerate() {
            assert!(
                (x - y).abs() < 1e-10,
                "Addition should be associative at index {}",
                i
            );
        }
    }
    ...
    fn test_mul_commutativity() {
        ...
        for (a, b) in test_cases {
            ...
            for (i, (x, y)) in mul_ab.iter().zip(mul_ba.iter()).enumerate() {
                assert!(
                    (x - y).abs() < 1e-10,
                    "Multiplication should be commutative at index {}: a: {:?}, b: {:?}",
                    i,
                    a,
                    b
                );
    ...
    fn test_mul_associativity() {
        ...
        for (i, (x, y)) in abc.iter().zip(abc2.iter()).enumerate() {
            assert!(
                (x - y).abs() < 1e-10,
                "Multiplication should be associative at index {}",
                i
            );
        }
    }
    ...
    fn test_string_operation_properties() {
        ...
        for string in test_strings {
            ...
            assert_eq!(
                upper1, upper2,
                "to_uppercase should be idempotent for: '{}'",
                string
            );
            ...
            assert_eq!(
                lower1, lower2,
                "to_lowercase should be idempotent for: '{}'",
                string
            );
        }
    }
    ...
    fn test_string_contains_properties() {
        ...
        for string in test_strings {
            if !string.is_empty() {
                assert!(
                    SimdStringOps::contains_simd(string, string),
                    "String should contain itself: '{}'",
                    string
                );
            }
        }
    ...
    fn test_cpu_capabilities_consistency() {
        ...
        assert_eq!(
            caps1.avx2, caps2.avx2,
            "AVX2 detection should be consistent"
        );
        assert_eq!(
            caps1.neon, caps2.neon,
            "NEON detection should be consistent"
        );
        assert_eq!(
            caps1.sse2, caps2.sse2,
            "SSE2 detection should be consistent"
        );
        assert_eq!(
            caps1.sse4_1, caps2.sse4_1,
            "SSE4.1 detection should be consistent"
        );

        assert_eq!(
            caps2.avx2, caps3.avx2,
            "AVX2 detection should be consistent"
        );
        assert_eq!(
            caps2.neon, caps3.neon,
            "NEON detection should be consistent"
        );
        assert_eq!(
            caps2.sse2, caps3.sse2,
            "SSE2 detection should be consistent"
        );
        assert_eq!(
            caps2.sse4_1, caps3.sse4_1,
            "SSE4.1 detection should be consistent"
        );
    }
