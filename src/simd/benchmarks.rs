...
use crate::simd::{get_best_simd_type, SimdOps, SimdStringOps, SimdType};
...
impl SimdBenchmarker {
    ...
    fn benchmark_numerical_operations(&mut self, size: usize) {
        ...
        self.benchmark_operation("sum_i64", size, iterations, || SimdOps::sum_i64(&data_i64));

        self.benchmark_operation("sum_f64", size, iterations, || SimdOps::sum_f64(&data_f64));
    ...
    fn benchmark_string_operations(&mut self, size: usize) {
        ...
        for (i, string) in test_strings.iter().enumerate() {
            if i >= 5 {
                break;
            } // Limit to first 5 strings for performance
        ...
        F: Fn() -> R,
    {
        ...
        println!(
            "✅ {} ({}): {:.2} ops/sec, {:.2}μs/op, {} elements",
            operation,
            simd_type,
            ops_per_second,
            avg_time_per_op.as_micros() as f64,
            data_size
        );
    }
        ...
        for (operation, results) in grouped {
            ...
            for result in results {
                println!(
                    ...
                    result.avg_time_per_op.as_micros() as f64,
                    result.simd_type
                );
            }
        }
    ...
    fn analyze_performance(&self) {
        ...
        println!(
            "Best performing operation: {} ({:.0} ops/sec)",
            best_operation, best_ops_per_second
        );
...
pub fn compare_simd_vs_scalar() {
    ...
    for size in data_sizes {
        ...
        println!(
            "Size: {}, SIMD: {:.2}ms, Scalar: {:.2}ms, Speedup: {:.2}x",
            size,
            simd_time.as_millis(),
