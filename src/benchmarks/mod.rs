...
impl PerformanceBenchmarks {
    ...
    pub fn benchmark_groupby_operations(
        frame: &TinyFrame,
        iterations: usize,
        ...
        for _ in 0..iterations {
            let _ =
                ParallelOps::parallel_groupby_sum(frame, group_keys.clone(), value_column.clone())?;
        }
        ...
        let start = Instant::now();
        for _ in 0..iterations {
            let _ = ParallelOps::parallel_groupby_mean(
                frame,
                group_keys.clone(),
                value_column.clone(),
            )?;
    ...
    pub fn benchmark_filtering_operations(
        frame: &TinyFrame,
        iterations: usize,
    ) -> PyResult<HashMap<String, f64>> {
        let mut results = HashMap::new();
    ...
    pub fn benchmark_sorting_operations(
        frame: &TinyFrame,
        iterations: usize,
    ) -> PyResult<HashMap<String, f64>> {
        let mut results = HashMap::new();
        ...
        results.insert(
            "parallel_sort_descending".to_string(),
            parallel_sort_desc_time,
        );
    ...
    pub fn benchmark_chunked_processing(
        frame: &TinyFrame,
        iterations: usize,
    ) -> PyResult<HashMap<String, f64>> {
        let mut results = HashMap::new();
        ...
        for chunk_size in chunk_sizes {
            ...
            results.insert(
                format!("chunked_groupby_chunk_{}", chunk_size),
                chunked_groupby_time,
            );
            ...
            results.insert(
                format!("chunked_filter_chunk_{}", chunk_size),
                chunked_filter_time,
            );
        }
    ...
    pub fn compare_performance_approaches(
        frame: &TinyFrame,
        iterations: usize,
    ) -> PyResult<HashMap<String, f64>> {
        let mut results = HashMap::new();
