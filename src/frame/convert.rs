...
pub fn from_dicts_impl(py: Python, records: &PyAny) -> PyResult<TinyFrame> {
    ...
    for col in &col_names {
        ...
        let final_col = if types_present.len() == 1 && !has_none {
            match types_present.iter().next().unwrap().as_ref() {
                "Int" => TinyColumn::Int(
                    value_enum_vals
                        .iter()
                        .map(|x| match x {
                            Some(ValueEnum::Int(i)) => *i,
                            _ => unreachable!(),
                        })
                        .collect(),
                ),
                "Float" => TinyColumn::Float(
                    value_enum_vals
                        .iter()
                        .map(|x| match x {
                            Some(ValueEnum::Float(f)) => *f,
                            _ => unreachable!(),
                        })
                        .collect(),
                ),
                "Bool" => TinyColumn::Bool(
                    value_enum_vals
                        .iter()
                        .map(|x| match x {
                            Some(ValueEnum::Bool(b)) => *b,
                            _ => unreachable!(),
                        })
                        .collect(),
                ),
                "Str" => TinyColumn::Str(
                    value_enum_vals
                        .iter()
                        .map(|x| match x {
                            Some(ValueEnum::Str(s)) => s.clone(),
                            _ => unreachable!(),
                        })
                        .collect(),
                ),
                "PyObject" => TinyColumn::PyObject(
                    value_enum_vals
                        .iter()
                        .map(|x| match x {
            ...
            match types_present.iter().next().unwrap().as_ref() {
                "Int" => TinyColumn::OptInt(
                    value_enum_vals
                        .iter()
                        .map(|x| {
                            x.as_ref().and_then(|v| match v {
                                ValueEnum::Int(i) => Some(*i),
                                _ => None,
                            })
                        })
                        .collect(),
                ),
                "Float" => TinyColumn::OptFloat(
                    value_enum_vals
                        .iter()
                        .map(|x| {
                            x.as_ref().and_then(|v| match v {
                                ValueEnum::Float(f) => Some(*f),
                                _ => None,
                            })
                        })
                        .collect(),
                ),
                "Bool" => TinyColumn::OptBool(
                    value_enum_vals
                        .iter()
                        .map(|x| {
                            x.as_ref().and_then(|v| match v {
                ...
                "Str" => TinyColumn::OptStr(
                    value_enum_vals
                        .iter()
                        .map(|x| {
                            x.as_ref().and_then(|v| match v {
                                ValueEnum::Str(s) => Some(s.clone()),
                                _ => None,
                            })
                        })
                        .collect(),
                ),
                "PyObject" => TinyColumn::OptPyObject(
                    value_enum_vals
                        .iter()
                        .map(|x| {
                            x.as_ref().and_then(|v| match v {
                                ValueEnum::PyObjectId(id) => Some(*id),
                                _ => None,
                            })
                        })
                        .collect(),
            ...
            if has_none {
                ...
                TinyColumn::Mixed(
                    value_enum_vals
                        .into_iter()
                        .map(|x| x.expect("Unexpected None in Mixed column"))
                        .collect(),
                )
            }
        };
        ...
        for (col_name, col_data) in &frame.columns {
            let val = match col_data {
                ...
                TinyColumn::OptMixed(v) => match &v[i] {
                    ...
                    Some(ValueEnum::PyObjectId(id)) => frame
                        .py_objects
                        .get(id)
                        .cloned()
                        .unwrap_or_else(|| py.None()),
