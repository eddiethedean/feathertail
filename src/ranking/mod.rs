...
impl RankingOps {
    ...
    pub fn rank_impl(frame: &TinyFrame, column: &str, method: RankMethod) -> PyResult<TinyFrame> {
        ...
        let ranks = match col {
            TinyColumn::Float(v) => {
                let indexed_values: Vec<(usize, f64)> =
                ...
                Self::calculate_ranks(&sorted_values, &method, v.len())
            }
            TinyColumn::Int(v) => {
                let indexed_values: Vec<(usize, i64)> =
                    v.iter().enumerate().map(|(i, &val)| (i, val)).collect();
                let mut sorted_values = indexed_values.clone();
                sorted_values.sort_by(|a, b| a.1.cmp(&b.1));

                // Convert to f64 for consistent ranking
                let float_values: Vec<(usize, f64)> = sorted_values
                    .iter()
                    .map(|(idx, val)| (*idx, *val as f64))
                    .collect();
                Self::calculate_ranks(&float_values, &method, v.len())
            }
            TinyColumn::OptFloat(v) => {
                let indexed_values: Vec<(usize, Option<f64>)> =
                    v.iter().enumerate().map(|(i, &val)| (i, val)).collect();
                let mut sorted_values = indexed_values.clone();
                sorted_values.sort_by(|a, b| match (a.1, b.1) {
                ...
                Self::calculate_ranks_optional(&sorted_values, &method, v.len())
            }
            TinyColumn::OptInt(v) => {
                let indexed_values: Vec<(usize, Option<i64>)> =
                    v.iter().enumerate().map(|(i, &val)| (i, val)).collect();
                let mut sorted_values = indexed_values.clone();
                sorted_values.sort_by(|a, b| match (a.1, b.1) {
                ...
                // Convert to f64 for consistent ranking
                let float_values: Vec<(usize, Option<f64>)> = sorted_values
                    .iter()
                    .map(|(idx, val)| (*idx, val.map(|v| v as f64)))
                    .collect();
                Self::calculate_ranks_optional(&float_values, &method, v.len())
            }
            _ => {
    ...
    pub fn pct_change_impl(frame: &TinyFrame, column: &str) -> PyResult<TinyFrame> {
            ...
            TinyColumn::OptFloat(v) => {
                ...
                for i in 1..v.len() {
                    match (v[i - 1], v[i]) {
                        ...
                        (Some(_prev), None) => pct_changes.push(None),
        ...
        let mut new_columns = frame.columns.clone();
        new_columns.insert(
            format!("{}_pct_change", column),
            TinyColumn::OptFloat(pct_changes),
        );
            ...
            RankMethod::Average => {
                ...
                while i < sorted_values.len() {
                    ...
                    while j < sorted_values.len()
                        && Self::f64_tied_for_rank(sorted_values[j].1, current_value)
                    {
                        j += 1;
                    }
    ...
    fn calculate_ranks_optional(
        ...
        let non_null_values: Vec<(usize, f64)> = sorted_values
            .iter()
